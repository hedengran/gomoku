// Authors: Gustav Hedengran, Lars Korduner
...
CALL {
    CALL {
        ...
        RETURN p, before_start, start, end, after_end, r1.direction AS direction
    } // all valid paths

    WITH *
        WHERE all(node IN nodes(p) WHERE node.state IS NULL OR node.state = $symbol) OR 
            all(node IN nodes(p) WHERE node.state IS NULL OR node.state <> $symbol) // exlude all paths that include both symbols since they cannot lead to victory
