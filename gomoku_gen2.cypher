CALL {
    ...
    OPTIONAL MATCH (before_start WHERE before_start.state IS NULL)-->(start)
    OPTIONAL MATCH (end)-->(after_end WHERE after_end.state IS NULL)
...
WITH p,
     CASE
     WHEN before_start IS NOT NULL AND after_end IS NOT NULL THEN 2
     WHEN before_start IS NOT NULL OR after_end IS NOT NULL THEN 1
     ELSE 0
     END AS openeded
...
WITH candidate, 
     ...
     CASE
        WHEN myScore = 5 THEN openeded + 1    // 6 and 5 are compensated for + 2 and + 1 in above size-projection
        WHEN otherScore = 4 THEN openeded
        ELSE 0
